        ...
        def lookup (envs : List[Environ], name : String) : Option[MValue] =
            envs match {
                case env :: envtail =>
                    val v = env.get (name)
                    if(v.isEmpty)
                      lookup(envtail, name)  // to allow recursion
                    else
                      v
                case _ =>
                    None
        ...
        instr match {
            ...
            case IEqual () =>
                opnds match {
                    ...
                    case MInt (r) :: MInt (l) :: opndstail =>
                        State (MBool (l == r) :: opndstail, envs, code)
                    case MList (r) :: MList (l) :: opndstail =>
                        State (MBool (l == r) :: opndstail, envs, code)
                    case _ =>
                        FatalError ("int and int, or bool and bool, or list and list expected in equal")
                }
            ...
            case INil ()  =>
                State (MList (List()) :: opnds, envs, code)

            case ICons () =>
                opnds match {
                    case MList (r) :: MInt (l) :: opndstail =>
                        State (MList (l :: r) :: opndstail, envs, code)
                    case _ =>
                        FatalError ("int and list expected in cons" + opnds)
                }

            case IHead () =>
                opnds match {
                    case MList (h :: t) :: opndstail =>
                        State (MInt (h) :: opndstail, envs, code)
                    case _ =>
                        FatalError ("(non-empty) list expected in head")
                }

            case ITail () =>
                opnds match {
                    case MList (h :: t) :: opndstail =>
                        State (MList (t) :: opndstail, envs, code)
                    case _ =>
                        FatalError ("(non-empty) list expected in tail")
                }

            case ILength () =>
                opnds match {
                    case MList (s) :: opndstail =>
                        State (MInt (s.size) :: opndstail, envs, code)
                    case _ =>
                        FatalError ("list expected in length")
                }
